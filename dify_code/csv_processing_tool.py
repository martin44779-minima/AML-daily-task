...
def process_csv_data_tool(csv_file_path: str = None, csv_content: str = None) -> Dict[str, Any]:
    """
    Dify工具函数：处理CSV数据，支持文件路径或内容输入
    ...
    """
    try:
        tool = CSVProcessingTool()

        # 如果提供了CSV内容字符串，先保存为临时文件
        temp_file_path = None
        if csv_content:
            # 创建临时文件来处理CSV内容
        ...
        logger.info(f"开始预处理CSV文件: {input_path}")
        ...
        tool.seen_trans_keys = set()
        tool.seen_id_pairs = set()  # 重置已见的id_columns组合
        ...
        chunk_iter = pd.read_csv(
            ...
            encoding='utf-8',
            header=None,
            names=list(tool.column_mapping.keys()),
            chunksize=tool.chunk_size,
        ...
        for chunk_idx, chunk_df in enumerate(chunk_iter):
            ...
            # 重命名列
            chunk_df.rename(columns=tool.column_mapping, inplace=True)
            ...
            if missing_columns:
                return {
                    "success": False,
                    "message": f"缺少必要字段: {missing_columns}",
                    "processed_count": 0,
                    "output_data": None
                }

            # 检查id_columns是否都存在
            missing_id_columns = [col for col in tool.id_columns if col not in chunk_df.columns]
            ...
            chunk_df = chunk_df.drop_duplicates(subset=tool.id_columns, keep='first')
            removed_duplicate_rows += initial_chunk_rows - len(chunk_df)

            # 过滤掉之前已处理过的组合
            chunk_df = chunk_df[~chunk_df[tool.id_columns].apply(tuple, axis=1).isin(tool.seen_id_pairs)]
            tool.seen_id_pairs.update(chunk_df[tool.id_columns].apply(tuple, axis=1))

            # 处理当前块
            processed_chunk = tool._process_chunk(chunk_df)
        ...
        for case_id, group in all_groups.items():
            ...
            chunk_results, _ = tool._aggregate_case_data(temp_grouped)
            if chunk_results:
        ...
        logger.info(f"  输出行数: {len(result_list)}")
        logger.info(f"  唯一ID组合数量: {len(tool.seen_id_pairs)}")
        ...
        return {
            "success": True,
            "message": f"预处理完成，共处理 {total_processed_rows} 行数据，{total_chunks} 个数据块，生成 {len(result_list)} 个案例",
            ...
            "unique_ids_count": len(tool.seen_id_pairs)
        }
...
if __name__ == "__main__":
    # 示例调用
    result = process_csv_data_tool(csv_file_path="./sample.csv")
